...
if [ $flag_peakCallingbygroup -gt 0 ]; then
    ...
    read -u 9
    {
        ...
        macs2 callpeak -t macs2_group_${group_id}_ip.bam -c macs2_group_${group_id}_input.bam -g hs -n macs2_group_${group_id} -p 0.05 -f BAM --nomodel
...
#peakCalling by individual sample
    ...
    read -u 9
    {
        macs2 callpeak -t ${sample_id}.ip*.bam -c ${sample_id}.input*.bam -g hs -n macs2_${sample_id} -p 0.05 -f BAM --nomodel
