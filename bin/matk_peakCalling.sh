...
if [ $flag_peakCallingbygroup -gt 0 ]; then
    ...
    do 
    {
        ...
        awk 'BEGIN{FS="\t";OFS="\t"}{print $1,$2,$3,$4,-log($5)/log(10)}' MATK_group_${group_id}.bed > MATK_group_${group_id}_normalized.bed
    }
    done 
else
    ...
    do
    {
        ...
        awk 'BEGIN{FS="\t";OFS="\t"}{print $1,$2,$3,$4,-log($5)/log(10)}' MATK_${sample_id}.bed > MATK_${sample_id}_normalized.bed
    }
    done
fi
wait
