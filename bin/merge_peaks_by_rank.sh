...
if [ $flag_peakCallingbygroup -gt 0 ]; then
    ...
    read -u 9
    {
        if [ $peakCalling_tools_count -gt 1 ]; then
            mergebed_by_rank ${group_id} rank_merged_group_${group_id}
        else
    ...
    wait
    mergebed_by_rank merged_group rank_merged_allpeaks
else
    ...
    {  
        ...
        if [ $peakCalling_tools_count -gt 1 ]; then
            mergebed_by_rank ${sample_id} rank_merged_sample_${group_id}_${sample_id}
        else
    ...
    read -u 9
    {
        mergebed_by_rank merged_sample_${group_id} rank_merged_group_${group_id}
        echo >&9
    }&
    done
    wait
    #mergebed_by_rank merged_sample rank_merged_allpeaks
