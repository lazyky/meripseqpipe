...
output_input_file = argv[6]
output_ip_file = argv[7]
def windows_fisher_test(input_count, ip_count, input_total_reads_count, ip_total_reads_count):
    ...
    input_oddsratio, input_pvalue = stats.fisher_exact([[input_count, ip_count], [input_total_reads_count, ip_total_reads_count]], 'less')
    ip_oddsratio, ip_pvalue = stats.fisher_exact([[input_count, ip_count], [site_input_rest_reads_count, site_ip_rest_reads_count]], 'greater')
...
with open (input_bin25_file) as input_bin25,open (ip_bin25_file) as ip_bin25:
    ...
    while True:
        ...
        if (input_pvalue < 0.05/peak_windows_number ):
            del input_line_list[-1]
            input_line_list.append(input_pvalue)
            input_line_list[1] = int(input_line_list[1])
            input_line_list[2] = int(input_line_list[2])
            input_bonferroni_filter_list.append([])
            input_bonferroni_filter_list[input_index] = input_line_list
            input_index += 1
...
input_bonferroni_peak = cluster_bin(input_bonferroni_filter_list[:])
ip_bonferroni_peak = cluster_bin(ip_bonferroni_filter_list[:])
"""Write the list of bonferroni_filter_peaks"""
with open(output_input_file,'w') as output_file:
    for data in input_bonferroni_peak:
