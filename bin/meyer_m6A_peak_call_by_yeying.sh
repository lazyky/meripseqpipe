...
awk -v bam="$INPUTbam" -v pre="$prefix" '{print "samtools view -b "bam" "$1 ">./"pre".tmp/input/"$1".bam"}' ./chrName.txt|xargs -iCMD -P$THREAD_NUM bash -c CMD
awk -v pre="$prefix"  '{print "bamToBed -split -i < ./"pre".tmp/input/"$1".bam>./"pre".tmp/input/"$1".bed"}' ./chrName.txt|xargs -iCMD -P$THREAD_NUM bash -c CMD
mkdir "$prefix.tmp/ip"
awk -v bam="$IPbam" -v pre="$prefix"  '{print "samtools view -b "bam" "$1 ">./"pre".tmp/ip/"$1".bam"}' ./chrName.txt|xargs -iCMD -P$THREAD_NUM bash -c CMD
awk -v pre="$prefix"   '{print "bamToBed -split -i < ./"pre".tmp/ip/"$1".bam>./"pre".tmp/ip/"$1".bed"}' ./chrName.txt|xargs -iCMD -P$THREAD_NUM bash -c CMD

awk -v pre="$prefix"  '{print "sortBed -i ./"pre".tmp/input/"$1".bed | intersectBed  -a genomebin/"$1".bin25.bed -b - -sorted -c > ./"pre".tmp/input/"$1".bin25.txt"}' ./chrName.txt|xargs -iCMD -P$THREAD_NUM bash -c CMD
awk -v pre="$prefix"  '{print "sortBed -i ./"pre".tmp/ip/"$1".bed | intersectBed  -a genomebin/"$1".bin25.bed -b - -sorted -c > ./"pre".tmp/ip/"$1".bin25.txt"}' ./chrName.txt|xargs -iCMD -P$THREAD_NUM bash -c CMD

echo "cal pval for each 25bp bin"
awk -v IPbam="$IPbam" -v INPUTbam="$INPUTbam" -v pre="$prefix" '{print "paste -d\\\"\\\\t\\\" ./"pre".tmp/ip/"$1".bin25.txt ./"pre".tmp/input/"$1".bin25.txt | perl cal_pval_for_bin.pl - "IPbam".readcounts.txt "INPUTbam".readcounts.txt> ./"pre".tmp/"$1".m6A.meyer.pval.txt"}' ./chrName.txt|xargs -iCMD -P$THREAD_NUM bash -c CMD
