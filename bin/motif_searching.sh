...
function motif_searching_by_pvalue()
{
    ...
    chrom_size=$5
    prefix=$6
    sort -k5,5 -g ${bed_file}| head -2000 | awk '{ print $1"\t"$2"\t"$3}' > ${prefix}.location
    ...
    shuffleBed -incl ${bed12} -seed 12345 -noOverlapping -i ${prefix}_bestpeaks.bed -g $chrom_size > ${prefix}_random_peak.bed
...
if [ $bed_count -gt 0 ]; then
    for bedfile in $(ls *.bed| grep -v $bed12_file)
    do
    {
        motif_searching_by_pvalue $bedfile $fasta_file $gtf_file $bed12_file chromsizes.file ${bedfile/.bed/}
    }
    done
fi
wait
