# nf-core/m6APipe: Configuration for other clusters
...
### Docker
Docker is a great way to run nf-core/m6APipe, as it manages all software installations and allows the pipeline to be run in an identical software environment across a range of systems.

Nextflow has [excellent integration](https://www.nextflow.io/docs/latest/docker.html) with Docker, and beyond installing the two tools, not much else is required - nextflow will automatically fetch the [nfcore/m6APipe](https://hub.docker.com/r/nfcore/m6APipe/) image that we have created and is hosted at dockerhub at run time.

To add docker support to your own config file, add the following:

```nextflow
docker.enabled = true
process.container = "nfcore/m6APipe"
```
...
process.container = "shub://nf-core/m6APipe"
```
...
```nextflow
singularity.enabled = true
process.container = "/path/to/nf-core-m6APipe.simg"
```
